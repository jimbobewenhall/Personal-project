...
class ADS1263:
    def __init__(self):
        self.rst_pin = ADS_config.RST_PIN
        self.cs_pin = ADS_config.CS_PIN
        self.drdy_pin = ADS_config.DRDY_PIN


    # Hardware reset
    def ADS1263_reset(self):
        ADS_config.digital_write(self.rst_pin, GPIO.HIGH)
        ADS_config.delay_ms(200)
        ADS_config.digital_write(self.rst_pin, GPIO.LOW)
        ADS_config.delay_ms(200)
        ADS_config.digital_write(self.rst_pin, GPIO.HIGH)
        ADS_config.delay_ms(200)
    ...
    def ADS1263_WriteCmd(self, reg):
        ADS_config.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        ADS_config.spi_writebyte([reg])
        ADS_config.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
    
    
    def ADS1263_WriteReg(self, reg, data):
        ADS_config.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        ADS_config.spi_writebyte([ADS1263_CMD['CMD_WREG'] | reg, 0x00, data])
        ADS_config.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
        
        
    def ADS1263_ReadData(self, reg):
        ADS_config.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        ADS_config.spi_writebyte([ADS1263_CMD['CMD_RREG'] | reg, 0x00])
        data = ADS_config.spi_readbytes(1)
        ADS_config.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
        return data
    ...
    def ADS1263_WaitDRDY(self):
        for i in range(0,400000,1):
            if(ADS_config.digital_read(self.drdy_pin) == 0):
                break
        if(i >= 400000):
            print ("Time Out ...\r\n")
    ...
    #The ADS_configuration parameters of ADC, gain and data rate
    def ADS1263_ADS_configADC(self, gain, drate):
        MODE2 = 0x80
        MODE2 |= (gain << 4) | drate
    ...
    #The ADS_configuration parameters of ADC2, gain and data rate
    def ADS1263_ADS_configADC2(self, gain, drate):
    ...
    def ADS1263_init(self):
        ...
        self.ADS1263_ADS_configADC(ADS1263_GAIN['ADS1263_GAIN_1'], ADS1263_DRATE['ADS1263_20SPS'])
        self.ADS1263_ADS_configADC2(ADS1263_ADC2_GAIN['ADS1263_ADC2_GAIN_1'], ADS1263_ADC2_DRATE['ADS1263_ADC2_10SPS'])
        return 0
    ...
    def ADS1263_Read_ADC_Data(self):
        ADS_config.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        while(1):
            ADS_config.spi_writebyte([ADS1263_CMD['CMD_RDATA1']])
            ADS_config.delay_ms(10)
            if(ADS_config.spi_readbytes(1)[0] & 0x40 != 0):
                break
        buf = ADS_config.spi_readbytes(5)
        ADS_config.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
    ...
    def ADS1263_Read_ADC2_Data(self):
        ...
        ADS_config.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        while(1):
            ADS_config.spi_writebyte([ADS1263_CMD['CMD_RDATA2']])
            ADS_config.delay_ms(10)
            if(ADS_config.spi_readbytes(1)[0] & 0x80 != 0):
                break
        buf = ADS_config.spi_readbytes(5)
        ADS_config.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
